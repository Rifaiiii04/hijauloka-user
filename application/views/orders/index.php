...
<div class="container mx-auto max-w-2xl py-12 min-h-[60vh]">
    ...
    <div class="flex justify-center mb-8 gap-2 flex-wrap">
        <?php foreach ($status_map as $key => $label): ?>
            <a href="?status=<?= $key ?>" class="px-4 py-2 rounded-full font-semibold text-sm transition-all
                <?= $status === $key ? 'bg-green-600 text-white shadow' : 'bg-gray-100 text-green-800 hover:bg-green-200' ?>
                "><?= $label ?></a>
        <?php endforeach; ?>
    </div>
    ...
    <?php else: ?>
        <div class="space-y-7">
            <?php foreach ($filtered_orders as $order): ?>
                ...
                ?>
                <div class="bg-white rounded-2xl shadow-lg p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 hover:shadow-xl transition-all duration-300 animate-fade-in">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-2 mb-1">
                                <span class="px-2 py-0.5 rounded-full text-xs font-semibold <?= $badge ?> capitalize"><?= $order['stts_pemesanan'] ?></span>
                                <span class="px-2 py-0.5 rounded-full text-xs font-semibold <?= $pay_badge ?> capitalize">Pembayaran: <?= $order['stts_pembayaran'] ?></span>
                            </div>
                    ...
                    <div class="flex flex-col gap-2 sm:gap-0 sm:flex-row sm:items-center">
                        <a href="<?= base_url('orders/detail/' . $order['id_order']) ?>" class="px-5 py-2 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition-all shadow text-sm flex items-center gap-2"><i class="fas fa-eye"></i> Detail</a>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
